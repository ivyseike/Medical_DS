...
from agents.agent import AgentDQN, HRLAgentDQN
...
# parser.add_argument('--data_folder', dest='data_folder', type=str, default='dataset/muzhi', help='folder to all data')
parser.add_argument('--data_folder', dest='data_folder', type=str, default='dataset/simulated', help='folder to all data')
...
parser.add_argument('--slot_err_prob', dest='slot_err_prob', default=0, type=float, help='the slot err probability')
...
parser.add_argument('--priority_replay', dest='priority_replay', default=1, type=int, help='')
parser.add_argument('--fix_buffer', dest='fix_buffer', default=0, type=int, help='')
parser.add_argument('--origin_model', dest='origin_model', default=1, type=int, help='0 for not mask')
# load NLG & NLU model
...
parser.add_argument('--lr', dest='lr', type=float, default=0.0005, help='lr for DQN')
...
parser.add_argument('-o', '--write_model_dir', dest='write_model_dir', type=str, default='./checkpoints/exp_models/KR-DQN/', help='write model to disk')
...
slot_set = text_to_dict('{}/slot_set.txt'.format(data_folder))  # all slots with symptoms + all disease + UNK
...
agent_params['data_folder'] = params['data_folder']
agent_params['kg_enabled'] = params['kg_enabled']
agent_params['hrl_enabled'] = params['hrl_enabled']
agent_params['data_folder'] = params['data_folder']
if params['hrl_enabled'] == False:
    ...
    agent = HRLAgentDQN(sym_dict, dise_dict, req_dise_sym_dict, dise_sym_num_dict, tran_mat, dise_sym_pro, sym_dise_pro, sp, act_set, slot_set, agent_params, static_policy=True)
...
def simulation_epoch(simulation_epoch_size, output=False):
    ...
    # simulation_reward_history.append(res["ave_reward"])
    # simulation_acc_history.append(res["success_rate"])
    # simulation_turn_history.append(res["ave_turns"])
...
def training(count):
    ...
    for episode in range(start_episode, count):
        ...
        if agt == 9:
            ...
            simulation_reward.append(res["ave_reward"])
            simulation_success_rate.append(res["success_rate"])
            ...
            # eval_reward_history.append(eval_res["ave_reward"])
            # eval_acc_history.append(eval_res["success_rate"])
